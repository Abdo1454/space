  ...
  fetch("data.json")
    ...
    .then((data) => {
      const techData = data.technology[index];
        ...
        if (techData) {
            techControl1.classList.toggle("active-4", index === 0);
            techControl2.classList.toggle("active-4", index === 1);
            techControl3.classList.toggle("active-4", index === 2);
            console.log(techData);
            techName.textContent = techData.name;
            techDescription.textContent = techData.description;
            imageTech.src = techData.images.portrait;
            imageTech.alt = techData.name;
        }
    }
        )
